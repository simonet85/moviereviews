...
def home(request):
    ...
    if searchTerm:
        # Standard textual queries :  case-insensitive match
        movies = Movie.objects.filter(title__icontains=searchTerm)
    else:
        movies = Movie.objects.all()  # query data from database and return a <QuerySet []>
    return render(request, 'home.html', {'searchTerm': searchTerm, 'movies': movies})


def about(request):
    return HttpResponse('<h1 class="display-1">About Page</h1>')
